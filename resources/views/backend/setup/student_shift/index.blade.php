                  ...
                  <tbody>
                      @foreach ($student_shifts as $k => $student_shift)
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('student_shift.delete',['student_shift'=>$student_shift->id]) }}" method="POST" id='myForm_{{ $student_shift->id }}'>
                                    @method('DELETE')
                                    @csrf
                                </form>
                                <a href="{{ route('student_shift.edit',['student_shift'=>$student_shift->id]) }}"  class="btn btn-info ">Edit</a>
                                <a href="javascript:void(0);" onClick='on_delete({{ $student_shift->id }});' class="btn btn-danger ">Delete</a>
                            </td>
